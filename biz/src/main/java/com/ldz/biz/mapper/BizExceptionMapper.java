...
public interface BizExceptionMapper extends Mapper<BizException> {
	...
	@Select("SELECT SUM( CASE WHEN info_check_status = '00' THEN 1 ELSE 0 END ) 'BMDSH', "
			...
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '002' or CODE = '902' or CODE = '903') AND zt = '00' GROUP BY sfzmhm) 'SFQRYC', "
			+ "SUM( CASE WHEN accept_status <> '20' and info_check_status='10' THEN 1 ELSE 0 END ) 'SLDQR', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '003' or CODE = '902' or CODE = '903') AND zt = '00' GROUP BY sfzmhm) 'SLQRYC', "
			+ "( SELECT COUNT(1) FROM trainee_test_info where subject ='科目一' and  (test_result is null or test_result = '') ) 'KM1YY', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE CODE LIKE '9__' AND kskm = '1' AND zt = '00'  GROUP BY sfzmhm) 'KM1YYYC', "
			...
			+ "(SELECT COUNT(1) FROM biz_exception WHERE CODE LIKE '9__' AND kskm = '2' AND zt = '00'  GROUP BY sfzmhm) 'KM2YYYC', "
			...
			+ "(SELECT COUNT(1) FROM biz_exception WHERE CODE LIKE '9__' AND kskm = '3' AND zt = '00' GROUP BY sfzmhm) 'KM3YYYC', "
			...
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '102' OR CODE = '103') AND kskm = '1' AND zt = '00' GROUP BY sfzmhm) 'KM1QRYC', "
			+ "SUM( CASE WHEN sec_sub NOT IN ('30', '40') THEN 1 ELSE 0 END ) 'KM2DQR', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '202' OR CODE = '203') AND kskm = '2' AND zt = '00' GROUP BY sfzmhm) 'KM2QRYC', "
			+ "SUM( CASE WHEN third_sub NOT IN ('30', '40') THEN 1 ELSE 0 END ) 'KM3DQR', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '302' OR CODE = '303') AND kskm = '3' AND zt = '00' GROUP BY sfzmhm) 'KM3QRYC', "
			+ "( SELECT COUNT(1) FROM trainee_test_info where subject ='科目四' and  (test_result is null or test_result = '') ) 'KM4YY', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE CODE LIKE '9__' AND kskm = '4' AND zt = '00' GROUP BY sfzmhm) 'KM4YYYC', "
			+ "SUM( CASE WHEN forth_sub NOT IN ('10', '20') THEN 1 ELSE 0 END ) 'KM4DQR' "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE (CODE = '402' OR CODE = '403') AND kskm = '4' AND zt = '00' GROUP BY sfzmhm) 'KM4QRYC', "

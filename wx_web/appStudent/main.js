...
let options = {
  ...
  beforeEach(to,from,next) {
    let um = localStorage.getItem('userInfo')
    let uminfo = JSON.parse(um)
    if(to.path == '/pages/dialogs/loginInterface' || to.path == '/pages/Welcome' || to.path == '/pages/user/uploadHeadImg'){
      next()
    }else{
       if(!um){
        console.log("没有登录")
        ...
        if(uminfo.headImg){//拦截没有上传头像的用户
          next()
        }else{
          next({
            path:'/pages/user/uploadHeadImg'
          })
        }
      }

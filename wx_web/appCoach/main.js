...
let options = {
  ...
  beforeEach(to,from,next) {
    ...
    let um = localStorage.getItem('coachUserInfo')
    if(to.path == '/pages/dialogs/loginInterface' || to.path == '/pages/Welcome'){
      next()
    }else{
      if(!um){
        console.log("没有登录")
        next({
           path:'/pages/dialogs/loginInterface'
        })
      }else{
           next()
      }
...
ui.extend({
    ...
    getUser(){//获取用户信息
      return JSON.parse(localStorage.getItem('coachUserInfo'))
    },
    /**getUserMess(callback){//网络数据请求 获取用户信息
      this.$http('POST',ui.getApp().apis.USERMESS,{},(res)=>{
        if(res.code==200 && res.result){
          ...
          localStorage.setItem('coachUserInfo',JSON.stringify(res.result.userInfo))
    ...
    $http(method,url,data,succesCallback,failCallback,completeCallback){//网路数据请求
      ...
      let accessTokenStr = localStorage.getItem("coachToken")
      console.log(url)

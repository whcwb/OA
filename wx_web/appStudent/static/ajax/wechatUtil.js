...
//wechatUtil.appId = 'wx73038475e21c2257';
wechatUtil.appId = 'wx192387463a314498';//明涛驾校正式版
wechatUtil.token = '';
wechatUtil.sign = '';
wechatUtil.code = '';
wechatUtil.openid = '';
...
//wechatUtil.authLoginUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+wechatUtil.appId+'&redirect_uri='+urls.url+'&response_type=code&scope=snsapi_base&state=debug&connect_redirect=1#wechat_redirect';
  wechatUtil.authLoginUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+wechatUtil.appId+'&redirect_uri='+urls.wxUrl+'&response_type=code&scope=snsapi_base&state=debug&connect_redirect=1#wechat_redirect'; 
wechatUtil.afterReady = '';
//存储Vue对象，用来在微信方法中，可以调用vue内容
wechatUtil.vueParent = null;
wechatUtil.getQueryString = function(name) {
    console.log(wechatUtil.authLoginUrl)
...
wechatUtil.initConfig = ()=>{
  ...
  let url = urls.url+urls.wechat.getJsApiSign+"?&timestamp="+wechatUtil.timestamp+"&url="+encodeURIComponent(curl)+'&nonceStr='+wechatUtil.nonceStr;
  $.ajax({
    url:url,
    type:'get',
    success:function(res){

...
public interface BizExceptionMapper extends Mapper<BizException> {
	...
	@Select("SELECT SUM( CASE WHEN info_check_status = '00' THEN 1 ELSE 0 END ) 'BMDSH', "
			...
			+ "SUM(CASE WHEN (SELECT COUNT(1) FROM trainee_test_info where subject ='科目一' and  (test_result is null or test_result = '') AND trainee_id = m.id)=1 THEN 1 ELSE 0 END) 'KM1YY', "
			...
			+ "SUM(CASE WHEN ( SELECT COUNT(1) FROM trainee_test_info where subject ='科目二' and  (test_result is null or test_result = '') AND trainee_id = m.id)=1 THEN 1 ELSE 0 END) 'KM2YY', "
			...
			+ "SUM(CASE WHEN (  SELECT COUNT(1) FROM trainee_test_info where subject ='科目三' and  (test_result is null or test_result = '') AND trainee_id = m.id)=1 THEN 1 ELSE 0 END) 'KM3YY', "
			...
			+ "SUM(CASE WHEN ( SELECT COUNT(1) FROM trainee_test_info where subject ='科目四' and  (test_result is null or test_result = '') AND trainee_id = m.id)=1 THEN 1 ELSE 0 END ) 'KM4YY', "

...
public interface BizExceptionMapper extends Mapper<BizException> {
	...
	@Select("SELECT SUM( CASE WHEN info_check_status = '00' THEN 1 ELSE 0 END ) 'BMDSH', "
			...
			+ "( select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE  kskm = '1' AND zt = '00'  GROUP BY sfzmhm ) t) 'KM1YYYC', "
			...
			+ "(select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE  kskm = '2' AND zt = '00'  GROUP BY sfzmhm) t) 'KM2YYYC', "
			...
			+ "(select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE  kskm = '3' AND zt = '00' GROUP BY sfzmhm) t) 'KM3YYYC', "
			...
			+ "(select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE  kskm = '4' AND zt = '00' GROUP BY sfzmhm) t) 'KM4YYYC', "

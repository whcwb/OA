...
public interface BizExceptionMapper extends Mapper<BizException> {
	...
	@Select("SELECT SUM( CASE WHEN info_check_status = '00' THEN 1 ELSE 0 END ) 'BMDSH', "
			...
			+ "( SELECT COUNT(1) FROM trainee_test_info m JOIN biz_exception c ON m.id_card_no=c.sfzmhm where subject ='科目一' and  (test_result is null or test_result = '') and c.zt='00') 'KM1YYYC', "
			...
			+ "(SELECT COUNT(1) FROM trainee_test_info m JOIN biz_exception c ON m.id_card_no=c.sfzmhm where subject ='科目二' and  (test_result is null or test_result = '') and c.zt='00') 'KM2YYYC', "
			...
			+ "(SELECT COUNT(1) FROM trainee_test_info m JOIN biz_exception c ON m.id_card_no=c.sfzmhm where subject ='科目三' and  (test_result is null or test_result = '') and c.zt='00') 'KM3YYYC', "

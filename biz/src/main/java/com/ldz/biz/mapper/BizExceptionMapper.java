	...
	@Select({"<script>",
        ...
        "<choose>",
		    ...
		    "</when>"  ,
					...
					"  AND   DATE_FORMAT(DATE_ADD(fir_sub_test_time, INTERVAL ${tvalue} DAY), '%Y-%m-%d') &lt;= CURRENT_DATE " ,
					" </when>",
			"<when test=\" tcolumn == 'sec_sub_test_time' and column=='sec_sub_payment_time'\"> " ,
			"  AND  DATE_FORMAT(DATE_ADD(sec_sub_test_time, INTERVAL ${tvalue} DAY), '%Y-%m-%d') &lt;= CURRENT_DATE " ,
			" </when>",
			"<when test=\" tcolumn == 'third_sub_test_time' and column=='third_sub_payment_time'\"> " ,
			"  AND   DATE_FORMAT(DATE_ADD(third_sub_test_time, INTERVAL ${tvalue} DAY), '%Y-%m-%d') &lt;= CURRENT_DATE " ,
			" </when>",
		    "<otherwise>",
		    	"AND ${tcolumn} &lt;= #{tvalue}",
		    "</otherwise>",

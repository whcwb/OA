...
public interface BizExceptionMapper extends Mapper<BizException> {
	...
	@Select("SELECT SUM( CASE WHEN info_check_status = '00' THEN 1 ELSE 0 END ) 'BMDSH', "
			...
			+ "SUM( CASE WHEN  (fir_sub_test_time is not null or  fir_sub_test_time != '')  AND  (fir_sub_payment_time is null or fir_sub_payment_time = '' ) THEN 1 ELSE 0 END ) 'KM1JF', "
			...
			+ "SUM( CASE WHEN (sec_sub_test_time is not null or  sec_sub_test_time != '')  AND  (sec_sub_payment_time is null or sec_sub_payment_time = '' ) THEN 1 ELSE 0 END ) 'KM2JF', "
			...
			+ "SUM( CASE WHEN (third_sub_test_time is not null or  third_sub_test_time != '')  AND  (third_sub_payment_time is null or third_sub_payment_time = '' ) THEN 1 ELSE 0 END ) 'KM3JF', "
			+ "(SELECT COUNT(1) FROM biz_exception WHERE CODE = '301' AND kskm = '3' AND zt = '00') 'KM3JFYC', "
			+ " SUM(CASE WHEN (SELECT COUNT(1) FROM trainee_test_info where subject ='科目一' and  (test_result is null or test_result = '') AND trainee_id = m.id AND m.`fir_sub` NOT IN ('30','40'))=1 THEN 1 ELSE 0 END)  'KM1DQR', "
			+ "(select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE CODE = '102' AND kskm = '1' AND zt = '00' GROUP BY sfzmhm) t) 'KM1QRYC', "
			+ "SUM(CASE WHEN ( SELECT COUNT(1) FROM trainee_test_info where subject ='科目二' and  (test_result is null or test_result = '') AND trainee_id = m.id AND m.`sec_sub` NOT IN ('30','40'))=1 THEN 1 ELSE 0 END) 'KM2DQR', "
			+ "(select count(1) from ( SELECT COUNT(1) FROM biz_exception WHERE CODE = '202' AND kskm = '2' AND zt = '00' GROUP BY sfzmhm) t) 'KM2QRYC', "
			+ "SUM(CASE WHEN (  SELECT COUNT(1) FROM trainee_test_info where subject ='科目三' and  (test_result is null or test_result = '') AND trainee_id = m.id AND m.`third_sub` NOT IN ('30','40'))=1 THEN 1 ELSE 0 END) 'KM3DQR', "
			...
			+ "SUM(CASE WHEN ( SELECT COUNT(1) FROM trainee_test_info where subject ='科目四' and  (test_result is null or test_result = '') AND trainee_id = m.id AND m.`forth_sub` NOT IN ('30','40'))=1 THEN 1 ELSE 0 END ) 'KM4DQR'  , "

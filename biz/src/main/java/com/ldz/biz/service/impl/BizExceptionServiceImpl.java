...
public class BizExceptionServiceImpl extends BaseServiceImpl<BizException, java.lang.String> implements BizExceptionService {
	...
	public void expJobSave(BizExceptionConfig config) {
		...
		if ("001".equals(config.getCode())){
			...
			kskm = "1";
			//科目一约考.即将考试还未缴科目一初考费
			students = baseMapper.getTraineeInfoByColumn("1", "=1", "fir_sub_payment_time", config.getDays().toString());
		}else if ("201".equals(config.getCode())){
			kskm = "2";
			//科目二约考.即将考试还未缴科目二初考费
			students = baseMapper.getTraineeInfoByColumn("1", "=1", "sec_sub_payment_time", config.getDays().toString());
		}else if ("301".equals(config.getCode())){
			kskm = "3";
			//科目三约考.即将考试还未缴科目三初考费
			students = baseMapper.getTraineeInfoByColumn("1", "=1", "third_sub_payment_time", config.getDays().toString());
